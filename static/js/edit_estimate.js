...
function updateGrandTotal() {
    const totals = Array.from(document.querySelectorAll('.total'))
        .map(el => parseFloat(el.textContent.replace('XAF ', '').replace(',', '')) || 0);
    ...
    document.getElementById('grandTotal').textContent = formatXAF(grandTotal + laborCost);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.material-row').forEach(row => {
        updateRowTotal({ target: row.querySelector('input[name="quantity[]"]') });
    });
    updateGrandTotal();
